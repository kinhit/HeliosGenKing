...
export function getMagnificApiKey(): string | null {
  const dbKey = getSetting("magnific_api_key");
  if (dbKey) return dbKey;
  const envKey = process.env.MAGNIFIC_API_KEY ?? "";
  return envKey || null;
}

export function setMagnificApiKey(key: string): void {
  setSetting("magnific_api_key", key);
}

export function deleteMagnificApiKey(): void {
