...
function createSchema(d: DatabaseSync): void {
  ...
  // Provider adapters may use a remote task ID that differs from the local
  // task ID exposed to the UI (for example, Magnific tasks are prefixed with
  // `magnific-`). Keep that mapping durable so pending jobs can resume after
  // a desktop restart.
  for (const column of [
    ...
    try {
      d.exec(`ALTER TABLE generations ADD COLUMN ${column[0]} ${column[1]}`);
    } catch {
