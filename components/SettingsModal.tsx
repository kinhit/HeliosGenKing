...
function ApiKeysPanel({
  ...
  const handleMagnificSave = async () => {
    if (!magnificInput.trim()) return;
    setMagnificSaving(true);
    setMagnificError(null);
    try {
      await onMagnificKeySave(magnificInput.trim());
      setMagnificInput("");
    } catch (e: unknown) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "28px" }}>
      ...
      <div>
        ...
        <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.28)", marginTop: "6px", lineHeight: 1.5 }}>
          {t("settings.apiKeys.description", "API keys are stored securely on the server and never exposed to the browser.")}
        </p>
      </div>
      ...
      <div
        style={{
          ...
          background: "rgba(245,182,66,0.04)", border: "1px solid rgba(245,182,66,0.14)", borderRadius: "12px",
        }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <span style={{ width: "28px", height: "28px", borderRadius: "7px", background: "rgba(245,182,66,0.1)", border: "1px solid rgba(245,182,66,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <ProviderBrandIcon id="magnific" size={16} />
          </span>
          <div>
          ...
          {magnificKeyStatus === "set" && (
            <span style={{ marginLeft: "auto", fontSize: "10px", fontWeight: 600, color: "rgba(74,222,128,0.8)", background: "rgba(74,222,128,0.08)", border: "1px solid rgba(74,222,128,0.2)", borderRadius: "5px", padding: "2px 7px", letterSpacing: "0.04em" }}>
              SAVED
            </span>
          )}
        </div>
        {magnificKeyStatus === "unknown" ? (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <button onClick={onMagnificKeyDelete} style={{ padding: "7px 12px", borderRadius: "7px", border: "1px solid rgba(239,68,68,0.3)", background: "rgba(239,68,68,0.06)", color: "rgba(239,68,68,0.7)", cursor: "pointer", fontSize: "12px", fontWeight: 500, whiteSpace: "nowrap" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            <div style={{ display: "flex", gap: "8px" }}>
              <input type="password" placeholder={t("settings.magnific.keyPlaceholder", "Paste your Magnific API key")} value={magnificInput} onChange={(e) => setMagnificInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") handleMagnificSave(); }} style={{ ...INPUT_STYLE, flex: 1 }} />
              <button onClick={handleMagnificSave} disabled={!magnificInput.trim() || magnificSaving} style={{ padding: "7px 14px", borderRadius: "7px", border: "none", background: magnificInput.trim() ? "rgba(245,182,66,0.16)" : "rgba(255,255,255,0.04)", color: magnificInput.trim() ? "rgba(245,182,66,0.95)" : "rgba(255,255,255,0.25)", cursor: magnificInput.trim() ? "pointer" : "default", fontSize: "12px", fontWeight: 500, whiteSpace: "nowrap" }}>
                {magnificSaving ? t("settings.saving", "Saving…") : t("settings.save", "Save")}
              </button>
            </div>
...
function ImageModelsPanel({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "28px" }}>
      <div>
        <h2 style={{ fontSize: "17px", fontWeight: 600, color: "rgba(255,255,255,0.9)", margin: 0, lineHeight: 1.2 }}>
          {t("settings.imageModels", "Image Models")}
        </h2>
        <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.28)", marginTop: "6px", lineHeight: 1.5 }}>
          {t("settings.imageModels.description", "Choose which provider serves each image model. Azure-capable models show a deployment name field when Azure is selected.")}
        </p>
      </div>
      <ProviderLegend />
      <ModelGroup
        title={t("settings.imageModels", "Image Models")}
        accent="#fb923c"
        models={models}
        providers={providers}
...
function VideoModelsPanel({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "28px" }}>
      <div>
        <h2 style={{ fontSize: "17px", fontWeight: 600, color: "rgba(255,255,255,0.9)", margin: 0, lineHeight: 1.2 }}>
          {t("settings.videoModels", "Video Models")}
        </h2>
        <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.28)", marginTop: "6px", lineHeight: 1.5 }}>
          {t("settings.videoModels.description", "Choose which provider serves each video model.")}
        </p>
      </div>
      <ProviderLegend />
      <ModelGroup
        title={t("settings.videoModels", "Video Models")}
        accent="#5EEAD4"
        models={models}
        providers={providers}
        onProviderChange={onProviderChange}
...
function TextModelsPanel({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "28px" }}>
      ...
      <div>
        <h2 style={{ fontSize: "17px", fontWeight: 600, color: "rgba(255,255,255,0.9)", margin: 0, lineHeight: 1.2 }}>
          {t("settings.textModels", "Text Models")}
        </h2>
        <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.28)", marginTop: "6px", lineHeight: 1.5 }}>
          {t("settings.textModels.description", "Configure AI text models for chat. Azure Auto uses your Azure Foundry credentials from the API Keys tab.")}
        </p>
      </div>
...
export default function SettingsModal({ onClose }: SettingsModalProps) {
  ...
  const handleMagnificKeySave = async (key: string) => {
    const h = await authHeader();
    const res = await fetch("/api/settings/magnific-key", {
      method: "POST",
      headers: { ...h, "Content-Type": "application/json" },
      body: JSON.stringify({ magnificApiKey: key }),
    });
    if (!res.ok) throw new Error((await res.json()).error ?? "Failed to save");
        ...
        >
          ...
          <div style={{ padding: "0 10px 12px" }}>
            ...
            <select
              ...
              style={{ width: "100%", height: "30px", borderRadius: "7px", border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.75)", fontSize: "11px", padding: "0 7px" }}
            >
              <option value="en">{t("language.english", "English")}</option>
              <option value="zh-CN">{t("language.chinese", "简体中文")}</option>
            </select>
          </div>
          ...
          {NAV.map((item) => {
            ...
            return (
              ...
              >
                ...
                {{
                  "api-keys": t("settings.apiKeys", item.label),
                  "image-models": t("settings.imageModels", item.label),
                  "video-models": t("settings.videoModels", item.label),
                  "text-models": t("settings.textModels", item.label),
                  debug: t("settings.debug", item.label),
                }[item.id] ?? item.label}
              </button>
            );
          })}
